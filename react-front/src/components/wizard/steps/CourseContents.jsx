...
import { Card, Form, Input, Typography, List, Space, Alert, Button, Modal, Tooltip, Collapse, Badge, Progress, Popconfirm } from 'antd';
import { useSelector, useDispatch } from 'react-redux';
import { setCourseContent, deleteCourse } from '@/redux/wizardSlice';
import { BookOutlined, EditOutlined, ImportOutlined, CopyOutlined, CaretRightOutlined, DeleteOutlined } from '@ant-design/icons';
...
export default function CourseContents() {
    ...
    const [selectedCourseId, setSelectedCourseId] = useState(null);    const [bulkEditMode, setBulkEditMode] = useState('single'); // 'single' or 'all'
    ...
    const handleCopyContent = (fromCourseId) => {
        ...
        navigator.clipboard.writeText(content).then(() => {
            Modal.success({
                ...
                content: 'Ders içeriği panoya kopyalandı. Diğer derslerin içerik alanlarına yapıştırabilirsiniz.',
            });
        });
    };
    ...
    const getCompletionStatus = () => {
        ...
        const filledContents = courses.filter(course =>
            courseContents[course.id] && courseContents[course.id].trim() !== ''
        ).length;

        return {
            percentage: totalCourses > 0 ? Math.round((filledContents / totalCourses) * 100) : 0,
            filledContents,
            totalCourses
        };
    ...
    const collapseItems = courses.map(course => {
        const hasContent = courseContents[course.id] && courseContents[course.id].trim() !== '';
                    ...
                    {hasContent && (
                        <div style={{ marginBottom: 8 }}>
                            ...
                            <Text type="secondary" style={{ fontSize: '12px' }}>
                                İçerik uzunluğu: {contentLength} karakter {isContentShort && "(Önerilen: en az 200 karakter)"}
                            </Text>
                        </div>
                    )}
                      <Form.Item
                        ...
                        help={hasContent ? "İçerik girildi ✓" : "Bu ders eşleştirmede kullanılmayacak"}
                    >
                        <TextArea
                            rows={4}
    ...
    return (
        <div>            <Card
            ...
            >                <Alert
                    ...
                    description={
                        <div>
                            <Paragraph>
                                ...
                                <strong> Sadece içeriği doldurduğunuz dersler eşleştirme adımında analiz edilecektir.</strong>
                                İstemediğiniz dersleri tamamen silebilir veya içeriklerini boş bırakabilirsiniz.
                                En az bir dersin içeriğini doldurmanız yeterlidir.
                            </Paragraph>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                                <Text strong>
                                    Tamamlanan: {status.filledContents}/{status.totalCourses} ({status.percentage}%)
                                </Text>
                                <Space>
                ...
                {/* Bulk Edit Modal */}
                <Modal
                    title={bulkEditMode === 'single' ? "Ders İçeriğini Düzenle" : "Tüm Derslere İçerik Ekle"}
                ...
                >
                    <div style={{ marginBottom: 16 }}>
                        {bulkEditMode === 'single' ? (
                            <Text>
                                Seçilen ders için içeriği düzenleyin.
                            </Text>
                        ) : (
                            <Alert
