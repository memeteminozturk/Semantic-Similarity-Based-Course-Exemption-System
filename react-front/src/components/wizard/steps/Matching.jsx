...
import { Card, Button, Table, Tag, Typography, Row, Col, Alert, Spin, Checkbox, Progress, Select, Collapse } from 'antd';
...
export default function Matching() {
    ...
    const expandedRowRender = (record) => {
        ...
        const candidateColumns = [
            ...
            },
            {
                ...
                key: 'exempt',
                render: (exempt) => (
                    <Tag color={exempt ? 'success' : 'default'} 
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'status',
            render: (_, record) => (
                <Tag color={record.isEligible ? 'success' : 'default'} icon={record.isEligible ? <CheckCircleOutlined /> : <CloseCircleOutlined />}>
                        ...
                        <Row gutter={[0, 16]}>
                            <Col span={24}>
                                ...
                                <Table
                                    ...
                                    expandable={{
                                        expandedRowRender,
                                        rowExpandable: (record) => record.allCandidates && record.allCandidates.length > 1,
                                    }}
                                    rowClassName={(record) => !record.isEligible ? 'ineligible-row' : ''}
                                />
                            </Col>
                        </Row>
                        <style jsx>{`
            ...
            `}</style>
                ...
                {loading && (
                    <div style={{ textAlign: 'center', padding: '40px 0' }}>
                        ...
                        <p style={{ fontSize: '12px', color: '#999' }}>Dersler tüm dahili derslerle karşılaştırılıyor, lütfen bekleyiniz...</p>
                    </div>
                )}
