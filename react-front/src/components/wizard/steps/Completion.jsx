...
export default function Completion() {
  ...
  const handleDownload = () => {
    ...
    try {
      ...
      messageApi.success('Dilekçe indiriliyor...');
    } catch (error) {
  ...
  const handleSendEmail = async values => {
    try {
      ...
      const courseContentsFileBlob = includeCourseContents
        ? getOldContentsFile()
        : null;
          ...
          <Row gutter={[0, 24]} justify="center">
            <Col span={24}>
              <Card
                title={
                  <>
                    <FileWordOutlined /> Dilekçe
                  </>
                }
                size="small"
              >
                <p style={{ textAlign: 'center' }}>
                  Muafiyet dilekçeniz hazır. Aşağıdaki seçeneklerden birini
                  kullanarak işlemi tamamlayabilirsiniz.
                </p>
                <div style={{ marginTop: 16 }}>
                  <p>
                    <strong>Dosya Adı:</strong> muafiyet_dilekcesi_
                    {personalInfo.studentId || 'ogrenci'}.docx
                  </p>
                  <p>
                    <strong>Boyut:</strong>{' '}
                    {pdfBlob ? Math.round(pdfBlob.size / 1024) : 0} KB
                  </p>
                  <p>
                  ...
                  <p>
                    <strong>İçerilen Dersler:</strong> {selectedMatches.length}{' '}
                    ders
                  </p>
                </div>
              </Card>
            ...
            <Col xs={24} md={12} style={{ textAlign: 'center' }}>
              ...
              >
                ...
                <p style={{ marginTop: 8 }}>
                  Dilekçeyi bilgisayarınıza indirin
                </p>
              </Card>
            </Col>
            ...
            <Col xs={24} md={12} style={{ textAlign: 'center' }}>
              <Card
                title={
                  <>
                    <SendOutlined /> E-posta ile Gönder
                  </>
                }
                size="small"
                style={{ height: '100%' }}
                ...
                >
                  ...
                  <Alert
                    message={
                      <div>
                        <div>
                          <strong>Konu:</strong> Muafiyet Başvurusu -{' '}
                          {personalInfo.fullName || 'Öğrenci'} (
                          {personalInfo.studentId || 'Öğrenci No'})
                        </div>
                        <div style={{ marginTop: 4 }}>
                          <strong>Kopya (CC):</strong>{' '}
                          {personalInfo.email || 'Sizin e-postanız'}
                        </div>
                      </div>
                    }
                    type="info"
