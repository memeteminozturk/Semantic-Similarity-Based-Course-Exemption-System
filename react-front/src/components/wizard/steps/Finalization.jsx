...
export default function Finalization() {
  ...
  const selectedCourses = matches
    ...
    .map(match => {
      ...
      const externalCourse = courses.find(
        course => course.code === match.courseCode
      );

      // Find internal course details from INTERNAL_COURSES
      const internalCourse = INTERNAL_COURSES.find(
        course => course.code === match.matchedWith
      );

      // Format credit info as T-U-K string
      const formatCredit = course => {
        if (
          course?.theory !== undefined &&
          course?.practice !== undefined &&
          course?.nationalCredit !== undefined
        ) {
    ...
    try {
      ...
      const personalInfoForApi = {
        firstName: personalInfo.fullName
          ...
          : '',
        lastName: personalInfo.fullName
    ...
    <div>
      <Card title="Dilekçe Oluşturma" bordered={false}>
        <Alert
          ...
          description={
            <Paragraph>
              Şimdiye kadar sağladığınız bilgiler ve seçtiğiniz dersler
              kullanılarak muafiyet başvuru dilekçeniz oluşturulacaktır.
        ...
        <Row gutter={[16, 24]}>
          <Col span={24}>
            <Card size="small" title="Kişisel Bilgiler">
              <Row gutter={16}>
                <Col xs={24} sm={12}>
                  <p>
                    <strong>Öğrenci No:</strong> {personalInfo.studentId}
                  </p>
                  <p>
                    <strong>Ad Soyad:</strong> {personalInfo.fullName}
                  </p>
                </Col>
                <Col xs={24} sm={12}>
                  <p>
                  ...
                  <p>
                    <strong>E-posta:</strong> {personalInfo.email}
                  </p>
                  <p>
                    <strong>Geçiş Türü:</strong>{' '}
                    {personalInfo.transferType === 'yatay'
                      ? 'Yatay Geçiş'
                      : 'Dikey Geçiş'}
                  </p>
                </Col>
          ...
          <Col span={24}>
            <Card size="small" title="Seçilen Dersler">
              {' '}
              <List
                    ...
                    <List.Item.Meta
                      ...
                      description={
                        <Space>
                          ...
                          <Text>
                            {item.int_code} - {item.int_name}
                          </Text>
                          <Text type="secondary">({item.int_credit})</Text>
                          <Text type="secondary">
                            (%{item.similarity_percent} benzerlik)
                          </Text>
                        </Space>
                      }
                    />
