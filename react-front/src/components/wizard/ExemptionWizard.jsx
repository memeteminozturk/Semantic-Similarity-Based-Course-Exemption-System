...
function ExemptionWizardContent() {
    ...
    const canProceed = () => {
        switch (activeStep) {
            ...
            case 1: // Transcript Step
                return transcriptFile && courses && courses.length > 0;            case 2: // Course Contents
                // En az bir dersin içeriği doldurulmuş olmalı
                return courses && courses.length > 0 &&
                    courses.some(course => courseContents[course.id] && courseContents[course.id].trim());
            case 3: // Matching
                return selectedMatches && selectedMatches.length > 0;            case 4: // Personal Info
                const { studentId, fullName, university, faculty, department, phone, email } = personalInfo;
                return studentId && fullName && university && faculty && department && phone && email && oldContentsFile;
            case 5: // Finalization
    ...
    const getValidationMessage = () => {
        switch (activeStep) {
            case 1:
                ...
                if (!courses.some(course => courseContents[course.id] && courseContents[course.id].trim()))
                    return 'En az bir dersin içeriğini doldurmalısınız. İçerik doldurulmayan dersler eşleştirme adımında görünmeyecektir.';
                break;
            case 3:
                ...
                if (!university) return 'Üniversitenizi girin';
                if (!faculty) return 'Fakülte/MYO/YO bilgisini girin';
                if (!department) return 'Bölümünüzü girin';
    ...
    const handleNext = () => {
        ...
        if (activeStep === 1 && transcriptStepRef.current) {
            ...
            // For transcript step, if courses were saved successfully, proceed immediately
            // Don't check canProceed as the state hasn't updated yet
            dispatch(nextStep());
            return;
        }
