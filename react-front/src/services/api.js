...
export async function matchCourses(transcript, oldContents) {
  try {
    ...
    if (oldContents.name)
      formData.append('old_contents_name', oldContents.name);
...
export async function generatePdf(personalInfo, selectedCourses) {
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text().catch(() => 'Unknown error');
      throw new Error(
        `Word document generation failed - ${response.status}: ${errorText}`
      );
    }

    const blob = await response.blob();
    console.log(
      'Word document generated successfully, size:',
      blob.size,
      'bytes'
    );
    return blob;
...
export async function bulkMatch(items) {
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text().catch(() => 'Unknown error');
      throw new Error(
        `Bulk similarity failed - ${response.status}: ${errorText}`
      );
    }
    const result = await response.json();
    return result;
