...
from .services import PdfGenerationService, SimilarityService
...
async def auto_match(req: AutoMatchRequest):
    ...
    for i, ext_code in enumerate(ext_codes):
        ...
        for j, sim in enumerate(course_sims):
            ...
            candidates.append(
                MatchCandidate(int_code=int_code, percent=percent, exempt=exempt)
            )
        ...
        result = AutoMatchResult(ext_code=ext_code, candidates=candidates)
        results.append(result)

    logger.info(
        "auto-match size=%d finished in %.1f ms",
        len(results),
...
async def generate_pdf(req: PdfGenerationRequest):
    ...
    try:
        ...
        if not req.personalInfo or not req.selectedCourses:
            raise HTTPException(
                status_code=400,
                detail="Personal info and selected courses are required",
            )

        if len(req.selectedCourses) == 0:
            raise HTTPException(
                status_code=400, detail="At least one course must be selected"
            )
        ...
        for i, course in enumerate(req.selectedCourses):
            if not course.ext_name:
                raise HTTPException(
                    status_code=400,
                    detail=f"Course at index {i} is missing ext_name field",
                )
            if not course.int_name:
                raise HTTPException(
                    status_code=400,
                    detail=f"Course at index {i} is missing int_name field",
                )
        ...
        for i, course in enumerate(selected_courses_list):
            logger.info(
                f"Course {i}: ext_name={course.get('ext_name')}, int_name={course.get('int_name')}"
            )
        ...
        logger.info(
            "Word document generated: %d courses, %d bytes, %.1f ms",
            len(req.selectedCourses),
            doc_size,
            generation_time,
        )
...
async def send_email(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid student_info JSON format"
            )
        ...
        if not exemption_document:
            raise HTTPException(
                status_code=400, detail="Exemption document is required"
            )
        ...
        course_contents_bytes = (
            await course_contents_file.read() if course_contents_file else None
        )
        ...
        logger.info(
            "Email sent: to=%s, attachments=%d, %.1f ms",
            to_email,
            sum(
                1
                for f in [exemption_doc_bytes, transcript_bytes, course_contents_bytes]
                if f
            ),
            email_time,
        )
