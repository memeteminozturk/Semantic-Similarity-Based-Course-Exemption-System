...
class SimilarityService:
    ...
    def __init__(self, model_name, threshold, repo):
        ...
        if not self._repo._cache:
            raise ValueError(
                "CourseRepository cache boş. 'await repo.load()' çalıştırılmadı."
            )
        self._int_codes = []
        self._int_embs = None
    ...
    def auto_match(self, ext_texts):
        ...
        sims = (ext_embs @ self._int_embs.T) / (
            np.linalg.norm(ext_embs, axis=1, keepdims=True)
            * np.linalg.norm(self._int_embs, axis=1)
        )
        return sims
...
class ExemptionWordBuilder:
    ...
    def build(
        ...
        upper_courses=None,
        generation_dt=None,
        template_path="Muafiyet_dilekcesi.docx",
    ):
        # Verileri Word formatına dönüştür
        data = ExemptionWordBuilder.map_data(
            personal_info, exemption_courses, upper_courses, generation_dt
        )
    ...
    def fill_table1(table, data):
        ...
        for i, (prev, exempt) in enumerate(
            zip(data["onceki_dersler"], data["muaf_dersler"])
        ):
    ...
    def generate_exemption_word(
        ...
        if timestamp:
            if isinstance(timestamp, str):
                try:
                    generation_dt = datetime.fromisoformat(
...
class PdfGenerationService:
    ...
    @staticmethod
    def generate_exemption_pdf(
        personal_info, muafiyet_courses, upper_courses=None, timestamp=None
    ):
