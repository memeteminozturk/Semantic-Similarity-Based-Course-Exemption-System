...
class EmailService:
    ...
    def __init__(self):
        ...
        if self.demo_mode:
            ...
            self.logger.info(
                f"Email service başlatıldı: {self.smtp_host}:{self.smtp_port} ({self.smtp_username})"
            )
        else:
            self.logger.warning(
                "Email service başlatıldı: Yapılandırma eksik, DEMO MODE'a geçiliyor"
            )
        ...
        try:
            ...
            if self.demo_mode or not self.is_configured:
                reason = (
                    "DEMO MODE aktif"
                    if self.demo_mode
                    else "Email yapılandırması eksik"
                )
                self.logger.info(f"DEMO MODE: {reason}")
                self.logger.info(f"DEMO MODE: Email would be sent to {to_email}")
                self.logger.info(
                    f"DEMO MODE: Student: {student_info.get('firstName', '')} {student_info.get('lastName', '')}"
                )
            ...
            if cc_email:
                msg["Cc"] = cc_email
            msg["Subject"] = (
                f"Muafiyet Başvurusu - {student_info.get('firstName', '')} {student_info.get('lastName', '')} ({student_info.get('studentNumber', '')})"
            )
            ...
            try:
                ...
                self.logger.warning(
                    "aiosmtplib not available, falling back to demo mode"
                )
                return {
                    "success": True,
    ...
    def _create_email_body(
        self, student_info: Dict[str, Any], custom_message: Optional[str] = None
    ) -> str:
        """Create HTML email body"""

        student_name = (
            f"{student_info.get('firstName', '')} {student_info.get('lastName', '')}"
        )
